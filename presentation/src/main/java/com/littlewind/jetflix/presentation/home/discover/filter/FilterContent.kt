package com.littlewind.jetflix.presentation.home.discover.filter
...
import com.littlewind.jetflix.presentation.home.discover.filter.option.*

@Composable
fun FilterBottomSheetContent(
    filterState: FilterMovieParams,
    genres: List<Genre>,
    onFilterStateChanged: (FilterMovieParams) -> Unit
) {
    Spacer(modifier = Modifier.padding(top = 4.dp))
    filterState.toFilterOptions(genres).forEach { filterOption ->
        filterOption.Render {
...
fun FilterMovieParams.toFilterOptions(genres: List<Genre>): List<FilterOption<*>> = buildList {
